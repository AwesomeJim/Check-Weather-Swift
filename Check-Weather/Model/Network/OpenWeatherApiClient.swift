...
class OpenWeatherApiClient {
    ...
    // MARK: - Fetch Weather Forecast - using a city name
    ...
    // MARK: - Fetch Weather Forecast - using a Coordinates
    class func fetchWeatherForecast(latitude:CLLocationDegrees,longitude:CLLocationDegrees, _ completion:@escaping(_ success:Bool, _ foreCastList : [WeatherItemModel]?, _ message:String?)-> Void) {
        //re-trive the API key form our info list
        guard let apiKey = Bundle.main.infoDictionary?["API_KEY"] as? String else {
            print("Un-Able to acces API Key")
            completion(false,nil,"Invalid Service Config.")
        ...
        //1. create url
        let url = OpenWeatherApiClient.Endpoints.forecastCoordinates(lat, log, apiKeyParam).url
        print(url)
